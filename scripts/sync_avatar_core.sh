...
if [ "${1:-}" = "--check" ]; then
  if diff -q <(body "$VENDORED") <(body "$upstream_raw") >/dev/null; then
    echo "✔ docs/avatar_core.js 본문이 원본과 일치합니다 ($source_desc)"
  else
    echo "✘ docs/avatar_core.js 가 원본보다 뒤처졌습니다 ($source_desc)" >&2
    diff <(body "$VENDORED") <(body "$upstream_raw") | head -40 >&2
    echo "→ ./scripts/sync_avatar_core.sh 로 갱신한 뒤 커밋하세요." >&2
    exit 1
  fi
  exit 0
fi
